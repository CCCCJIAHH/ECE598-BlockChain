...
use crate::types::ledger_state::{BlockState, ico};
...
fn main() {
    ...
    let transaction_pool = TransactionMempool::new();
    ...
    let worker_ctx = network::worker::Worker::new(p2p_workers, msg_rx, &server, &blockchain, &transaction_pool, &block_state);
    worker_ctx.start();

    // start the miner
    let (miner_ctx, miner, finished_block_chan) = miner::new(&blockchain, &transaction_pool);
    let miner_worker_ctx = miner::worker::Worker::new(&server, finished_block_chan, &blockchain, &block_state);
    miner_ctx.start();
    miner_worker_ctx.start();
    let (tx_gen_ctx, tx_gen) = types::transaction_generator::new(&server, &transaction_pool, &blockchain, &block_state);
    tx_gen_ctx.start();
